...
def conv_block(in_dims,out_dims,activation_function):
    model = nn.Sequential(
...
def conv_trans_block(in_dim,out_dim,activation_function):
    model = nn.Sequential(
        nn.ConvTranspose2d(in_channels = in_dim, out_channels = out_dim, kernel_size = 3, stride =2, padding=1),
...
def conv_block_3(in_dims,out_dims,activation_function):
    model = nn.Sequential(
        conv_block(in_dims,out_dims,activation_function),
        conv_block(in_dims, out_dims,activation_function)
        nn.Conv2d(in_channels= in_dims, out_channels = out_dims,kernel_size = kernel_size, stride= 1,padding=1),
